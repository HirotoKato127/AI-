...
function initializeAdFilters() {
  ...
  metricSelect?.addEventListener('change', (e) => {
    ...
    if (metricTitle) {
      const titleLabel = lineMetric === 'initialInterviewRate'
        ? '初回面談設定率'
        : lineMetric === 'retention30'
          ? '定着率'
          : '決定率';
...
async function loadAdPerformanceData() {
  const mockData = [
    ...
    { id: 37, mediaName: 'ビズリーチ', applications: 178, validApplications: 150, initialInterviews: 102, offers: 47, hired: 35, retention30: 90.8, refund: 21000, period: '2024-08' },
    { id: 38, mediaName: 'ビズリーチ', applications: 184, validApplications: 155, initialInterviews: 106, offers: 49, hired: 36, retention30: 91.5, refund: 22000, period: '2024-09' },
    { id: 39, mediaName: 'ビズリーチ', applications: 192, validApplications: 160, initialInterviews: 110, offers: 52, hired: 38, retention30: 92.0, refund: 23000, period: '2024-11' }
...
function updateAdSummary(data) {
  ...
  setText('adSummaryTotalApplications', totalApps ? formatNumber(totalApps) : '-');
  setText('adSummaryTotalValid', totalValid ? formatNumber(totalValid) : '-');
...
function renderAdCharts(aggregatedData, rawData = adState.filtered) {
  ...
  const rates = aggregatedData.map(d => d[lineMetric]).filter(v => Number.isFinite(v));
  let yMin = 0, yMax = 100;
  if (rates.length) {
  ...
  const metricLabel = lineMetric === 'initialInterviewRate'
    ? '初回面談設定率'
    : lineMetric === 'retention30'
      ? '定着率'
      : '決定率';
  decisionLineChart = new Chart(lineCanvas.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: ctx => `${ctx.dataset.label}: ${formatPercent(ctx.parsed.y || 0)} (${metricLabel})` } }
      },
...
function renderMetricTable(labels, mediaMap) {
  ...
  const rows = Object.keys(mediaMap).sort().map(name => {
    const cells = labels.map(lbl => {
      ...
      return `<td class="px-3 py-2 text-right text-sm text-slate-700">${Number.isFinite(v) ? formatPercent(v) : '-'}</td>`;
    });
    return `<tr>
...
function updateContractInfo(data) {
  ...
  box.innerHTML = `
    ...
    <div class="text-sm text-slate-600">契約料金: ${contractMock.price}</div>
    <div class="text-sm text-slate-600">契約の仕組み: ${contractMock.billing}</div>
    <div class="text-sm text-slate-600">更新・解約: ${contractMock.autoRenew}</div>
    <div class="text-xs text-slate-500 mt-1">契約条件のメモをここに追記できます（モック）</div>
  `;
}
