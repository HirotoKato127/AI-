...
app.get("/api/clients", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "莨∵･ｭ荳隕ｧ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
async function handleClientsKpi(req, res) {
  ...
  try {
    ...
    res.status(500).json({ error: "邏ｹ莉句・莨∵･ｭ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
}
...
app.post("/api/clients", async (req, res) => {
  ...
  if (!name) {
    res.status(400).json({ error: "莨∵･ｭ蜷阪′蠢・ｦ√〒縺吶・ });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "莨∵･ｭ縺ｮ逋ｻ骭ｲ縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
const deleteClientHandler = async (req, res) => {
  ...
  if (!id) {
    res.status(400).json({ error: "ID縺悟ｿ・ｦ√〒縺吶・ });
    return;
  }
  ...
  try {
    ...
    if (rowCount === 0) {
      res.status(404).json({ error: "謖・ｮ壹＆繧後◆莨∵･ｭ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縲・ });
    } else {
      res.json({ success: true, message: "莨∵･ｭ繧貞炎髯､縺励∪縺励◆縲・ });
    }
  } catch (error) {
    console.error("Failed to delete client", error);
    res.status(500).json({ error: "莨∵･ｭ縺ｮ蜑企勁縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
};
...
app.put("/api/clients", async (req, res) => {
  ...
  const id = payload.id;
  if (!id) {
    res.status(400).json({ error: "莨∵･ｭID縺悟ｿ・ｦ√〒縺吶・ });
    return;
  }
  ...
  if (updates.length === 0) {
    res.status(400).json({ error: "譖ｴ譁ｰ蜀・ｮｹ縺後≠繧翫∪縺帙ｓ縲・ });
    return;
  }
  ...
  try {
    ...
    if (rows.length === 0) {
      res.status(404).json({ error: "莨∵･ｭ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縲・ });
      return;
    }
    res.json({ item: rows[0] });
  } catch (error) {
    console.error("Failed to update client", error);
    res.status(500).json({ error: "莨∵･ｭ諠・ｱ縺ｮ譖ｴ譁ｰ縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.get("/api/system-options", async (req, res) => {
  ...
  if (!key) {
    res.status(400).json({ error: "key縺悟ｿ・ｦ√〒縺吶・ });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "險ｭ螳壹・蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.put("/api/system-options", async (req, res) => {
  ...
  if (!key || !options) {
    res.status(400).json({ error: "key縺ｨoptions縺悟ｿ・ｦ√〒縺吶・ });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "險ｭ螳壹・譖ｴ譁ｰ縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
const REPORT_STATUS_FLAGS = [
  { label: "LINE蝣ｱ蜻頑ｸ医∩", column: "line_reported" },
  { label: "蛟倶ｺｺ繧ｷ繝ｼ繝亥渚譏貂医∩", column: "personal_sheet_reflected" },
  { label: "隲区ｱよ嶌騾∽ｻ俶ｸ医∩", column: "invoice_sent" },
];
...
function buildCandidateNameKeySql(alias) {
  return `LOWER(REGEXP_REPLACE(COALESCE(${alias}.candidate_name, ''), '[\\s縲]+', '', 'g'))`;
}
...
function buildLegacyHearingMemo(hearing = {}) {
  const legacyFields = [
    { key: "relocation", label: "霆｢螻・ },
    { key: "desiredArea", label: "蟶梧悍繧ｨ繝ｪ繧｢" },
    { key: "timing", label: "霆｢閨ｷ譎よ悄" },
    { key: "desiredJob", label: "蟶梧悍閨ｷ遞ｮ" },
    { key: "firstInterviewMemo", label: "蛻晏屓髱｢隲・Γ繝｢" },
    { key: "currentIncome", label: "迴ｾ蟷ｴ蜿・ },
    { key: "desiredIncome", label: "蟶梧悍蟷ｴ蜿・ },
    { key: "reason", label: "霆｢閨ｷ逅・罰繝ｻ霆｢閨ｷ霆ｸ" },
    { key: "interviewPreference", label: "髱｢謗･蟶梧悍譌･" },
    { key: "recommendationText", label: "謗ｨ阮ｦ譁・ },
    { key: "otherSelections", label: "莉也､ｾ驕ｸ閠・憾豕・ },
  ];
  return legacyFields
...
function mapCandidateUpdateColumns(payload = {}) {
  ...
  return {
    ...
    phone: payload.phone ?? null,
    email: payload.email ?? null,
    birthday: normalizeDate(payload.birthday || payload.birthDate), // 菫ｮ豁｣: birthDate繧ょ女縺台ｻ倥￠繧・
    age:
...
async function replaceCandidateAppProfile(client, candidateId, payload = {}) {
  ...
  const fields = {
    ...
    strengths: payload.skills || null, // payload.skills -> strengths (莉ｮ繝槭ャ繝斐Φ繧ｰ)
...
function mapCandidate(row, extras = {}) {
  ...
  return {
    ...
    advisorName: row.partner_name, // 諡・ｽ薙い繝峨ヰ繧､繧ｶ繝ｼ縺ｯ partner_name 縺ｫ菫晏ｭ倥＆繧後※縺・ｋ
    ...
    partnerName: row.partner_name ?? detail.partnerName, // 蠕梧婿莠呈鋤諤ｧ縺ｮ縺溘ａ邯ｭ謖・
    introductionChance:
...
app.get("/api/candidates", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "蛟呵｣懆・ｸ隕ｧ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.get("/api/candidates/:id", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      res.status(404).json({ error: "蛟呵｣懆・′隕九▽縺九ｊ縺ｾ縺帙ｓ縲・ });
      return;
    }
    ...
    res.status(500).json({ error: "蛟呵｣懆・ｩｳ邏ｰ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.get("/api/settings/kintone", async (_req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "kintone險ｭ螳壹・蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.put("/api/candidates/:id", async (req, res) => {
  ...
  try {
    ...
    if (Number.isNaN(candidateId)) {
      res.status(400).json({ error: "蛟呵｣懆・D縺御ｸ肴ｭ｣縺ｧ縺吶・ });
      return;
    }
    const payload = req.body || {};
    ...
    res.status(500).json({ error: "蛟呵｣懆・・譖ｴ譁ｰ縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.post("/api/settings/kintone/test", async (req, res) => {
  ...
  if (!kintoneSubdomain || !kintoneAppId || !kintoneApiToken) {
    res.status(400).json({ error: "繧ｵ繝悶ラ繝｡繧､繝ｳ縲√い繝励ΜID縲、PI繝医・繧ｯ繝ｳ繧貞・蜉帙＠縺ｦ縺上□縺輔＞縲・ });
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.status(400).json({
        error: `謗･邯壹↓螟ｱ謨励＠縺ｾ縺励◆・・{response.status}・荏,
    ...
    res.json({
      success: true,
      message: `謗･邯壹↓謌仙粥縺励∪縺励◆縲ゑｼ亥叙蠕嶺ｻｶ謨ｰ ${body.records?.length ?? 0} 莉ｶ・荏,
    });
  } catch (error) {
    console.error("Failed to test kintone connection", error);
    res.status(500).json({ error: "謗･邯壹ユ繧ｹ繝井ｸｭ縺ｫ繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆縲・ });
  }
});
...
app.put("/api/settings/kintone", async (req, res) => {
  ...
  if (!kintoneSubdomain || !kintoneAppId) {
    res.status(400).json({ error: "繧ｵ繝悶ラ繝｡繧､繝ｳ縺ｨ繧｢繝励ΜID縺ｯ蠢・医〒縺吶・ });
    return;
  }
  ...
  try {
    ...
    if (!tokenToUse) {
      res.status(400).json({ error: "API繝医・繧ｯ繝ｳ繧貞・蜉帙＠縺ｦ縺上□縺輔＞縲・ });
      return;
    }
    ...
    res.status(500).json({ error: "kintone險ｭ螳壹・菫晏ｭ倥↓螟ｱ謨励＠縺ｾ縺励◆縲・ });
  } finally {
    client.release();
  }
});
...
app.get("/api/candidates/:id/educations", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "蟄ｦ豁ｴ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆" });
  }
});
...
app.put("/api/candidates/:id/educations", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "蟄ｦ豁ｴ縺ｮ菫晏ｭ倥↓螟ｱ謨励＠縺ｾ縺励◆" });
  } finally {
    client.release();
  }
});
...
app.get("/api/candidates/:id/work-histories", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "閨ｷ豁ｴ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆" });
  }
});
...
app.put("/api/candidates/:id/work-histories", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "閨ｷ豁ｴ縺ｮ菫晏ｭ倥↓螟ｱ謨励＠縺ｾ縺励◆" });
  } finally {
    client.release();
  }
});
...
function formatDateJPForPdf(value) {
  ...
  return `${d.getFullYear()}蟷ｴ${d.getMonth() + 1}譛・;
}
...
app.get("/api/candidates/:id/resume.pdf", async (req, res) => {
  ...
  try {
    ...
    if (!candidates.length) {
      res.status(404).json({ error: "蛟呵｣懆・′隕九▽縺九ｊ縺ｾ縺帙ｓ" });
      return;
    }
    ...
    const data = {
      ...
      educationsArray: educations.map(e => ({
        ...
        graduationStatus: e.graduation_status || "蜊呈･ｭ",
      })),
    ...
    res.status(500).json({ error: "螻･豁ｴ譖ｸPDF縺ｮ逕滓・縺ｫ螟ｱ謨励＠縺ｾ縺励◆" });
  } finally {
    client.release();
  }
});
...
app.get("/api/candidates/:id/cv.pdf", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "閨ｷ蜍咏ｵ梧ｭｴ譖ｸPDF縺ｮ逕滓・縺ｫ螟ｱ謨励＠縺ｾ縺励◆" });
  } finally {
    client.release();
  }
});
...
app.get("/api/settings/screening-rules", (req, res) => {
  // TODO: DB縺九ｉ蜿門ｾ励☆繧九ｈ縺・↓螳溯｣・
  res.json([]);
});

app.get("/api/members", (req, res) => {
  res.json([
    { id: 1, name: "邂｡逅・・螟ｪ驛・, email: "admin@example.com", role: "admin", is_admin: true },
    { id: 30, name: "繝・せ繝井ｸ闊ｬ", email: "test@example.com", role: "member", is_admin: false },
    { id: 2, name: "蝟ｶ讌ｭ 闃ｱ蟄・, email: "sales@example.com", role: "advisor", is_admin: false },
  ]);
});
