...
function mapRoleToDepartment(role) {
  ...
  if (!r) return "";
  if (r.includes("caller") || r.includes("call") || r.includes("teleapo") || r.includes("\u30C6\u30EC\u30A2\u30DD") || r.includes("\u67B6\u96FB") || r.includes("cs") || r.includes("\u30AB\u30B9\u30BF\u30DE\u30FC") || r.includes("\u30B5\u30DD\u30FC\u30C8")) {
    return "cs";
  }
  if (r.includes("marketer") || r.includes("marketing") || r.includes("market") || r.includes("\u30DE\u30FC\u30B1") || r.includes("\u30DE\u30FC\u30B1\u30C6\u30A3\u30F3\u30B0")) {
    return "marketing";
  }
  if (r.includes("advisor") || r.includes("sales") || r.includes("\u55B6\u696D") || r.includes("\u30A2\u30C9\u30D0\u30A4\u30B6\u30FC")) {
    return "sales";
  }
  return "";
}
...
const dailyYieldCache = /* @__PURE__ */ new Map();
...
const csTeleapoCache = /* @__PURE__ */ new Map();
...
const marketingValidAppCache = /* @__PURE__ */ new Map();
...
async function fetchCompanyEmployeePlannedKpis({ baseDate }) {
  const date = baseDate || isoDate(/* @__PURE__ */ new Date());
  const json = await fetchJson(`${KPI_API_BASE}${KPI_YIELD_PATH}`, {
    from: date,
    to: date,
    scope: "company",
...
async function ensureCandidateDataset() {
  ...
  candidateLoadedAt = /* @__PURE__ */ new Date();
...
const DASHBOARD_YEARS = [(/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getFullYear() - 1, (/* @__PURE__ */ new Date()).getFullYear() - 2];
...
function resolveAdvisorGoal(periodId, dateStr, advisorName, targetKey) {
  ...
  if (daily && daily[targetKey] !== void 0 && daily[targetKey] !== null) {
  ...
  return value === void 0 || value === null ? null : num(value);
}
...
function computeTodayDiffsFromDaily() {
  const todayStr = isoDate(/* @__PURE__ */ new Date());
  const periodId = goalSettingsService.resolvePeriodIdByDate(todayStr, state.evaluationPeriods) || state.personalDailyPeriodId || state.personalEvaluationPeriodId;
  if (!periodId) return {};
  const periodData = state.personalDailyData[periodId];
  if (!periodData) return {};
  const prevDate = isoDate(new Date(new Date(todayStr).getTime() - 24 * 60 * 60 * 1e3));
...
async function loadCompanyTodayEmployeeKpi() {
  ...
  const advisorIds = rows.map((row) => row.advisorUserId).filter((id) => Number.isFinite(id) && id > 0);
...
async function persistPersonalDailyMsTargets(periodId, deptKey, metricKey) {
  ...
  const targetTotal = state.personalDailyMs.totals?.[deptKey]?.[metricKey] ?? getLastCumulativeTarget(dates, dailyTargets, deptKey, periodId);
  try {
...
async function handlePersonalDailyDistribute(event) {
  ...
  const datesForTotal = (() => {
    const range2 = resolvePersonalDailyDateRange(periodId, deptKey);
    return range2.startDate && range2.endDate ? enumerateDateRange(range2.startDate, range2.endDate) : [];
  })();
  const currentTotal = state.personalDailyMs.totals?.[deptKey]?.[metricKey] ?? getLastCumulativeTarget(datesForTotal, getPersonalDailyTargetMap(deptKey, metricKey), deptKey, periodId);
...
function renderPersonalDailyTable(periodId, dailyData = {}) {
  ...
  targetDepts.forEach((dept, deptIndex) => {
    ...
    const metrics = isRevenue ? [{ key: "revenue", label: "\u58F2\u4E0A\u91D1\u984D", targetKey: "revenue", isCurrency: true }] : getMetricsForDept(dept);
    if (!metrics.length) return;
    ...
    metrics.forEach((metricOption, metricIndex) => {
      ...
      const totalTargetValue = Number.isFinite(storedTotal) ? num(storedTotal) : getLastCumulativeTarget(dates, targetMap, dept, periodId) || (totalTarget > 0 ? totalTarget : 0);
...
async function persistCompanyMsTargets(deptKey, metricKey) {
  ...
  const targetTotal = state.companyMs.msTargetTotals?.[deptKey]?.[metricKey] ?? (dates.length ? getLastCumulativeTargetForCompany(dates, dailyTargets, deptKey) : sumTargetValues(dailyTargets));
  try {
    await goalSettingsService.saveMsTargets({
      scope: "company",
      departmentKey: deptKey,
...
function handleMsTargetInput(event) {
  ...
  const dates = state.companyMs.dates || [];
  state.companyMs.msTargetTotals[dept][metric] = dates.length ? getLastCumulativeTargetForCompany(dates, state.companyMs.msTargets[dept][metric], dept) : sumTargetValues(state.companyMs.msTargets[dept][metric]);
  renderCompanyMsTable();
...
function handleCompanyMsDistribute(event) {
  ...
  if (!deptKey || !metricKey || !periodId) return;
  const dates = state.companyMs.dates || [];
  const currentTotal = state.companyMs.msTargetTotals?.[deptKey]?.[metricKey] ?? (dates.length ? getLastCumulativeTargetForCompany(dates, getCompanyMsTargetMap(deptKey, metricKey), deptKey) : sumTargetValues(getCompanyMsTargetMap(deptKey, metricKey)));
  const input = prompt("\u6700\u7D42\u76EE\u6A19\u5024\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044", currentTotal ? String(currentTotal) : "");
  if (input === null) return;
  const total = Number(input);
  if (!Number.isFinite(total) || total < 0) return;
...
function renderCompanyMsTable() {
  ...
  MS_DEPARTMENTS.forEach((dept, index) => {
    ...
    const metricCell = isRevenue ? `<th scope="row" class="kpi-v2-sticky-label kpi-v2-ms-metric" rowspan="3">
           <div class="ms-metric-cell">
             <span>${metricLabel}</span>
             ${distributeButton}
           </div>
         </th>` : `<th scope="row" class="kpi-v2-sticky-label kpi-v2-ms-metric" rowspan="3">
           <div class="ms-metric-cell">
    ...
    const totalTarget = Number.isFinite(state.companyMs.msTargetTotals?.[dept.key]?.[metricKey]) ? num(state.companyMs.msTargetTotals?.[dept.key]?.[metricKey]) : getLastCumulativeTargetForCompany(dates, targetMap, dept.key);
    const msCells = dates.map((date, idx) => {
      const isDisabled = isDateBeforeDeptStart(date, dept.key) || isDateAfterDeptEnd(date, dept.key);
      if (isDisabled) return `<td class="ms-cell-disabled"></td>`;
...
async function persistPersonalMsTargets(periodId, deptKey, memberId, metricKey) {
  ...
  const targetTotal = getPersonalMsTargetTotal(deptKey, memberId, metricKey) ?? getLastCumulativeTarget(dates, dailyTargets, deptKey, periodId);
  try {
    await goalSettingsService.saveMsTargets({
      scope: "personal",
      departmentKey: deptKey,
...
function handlePersonalMsDistribute(event) {
  ...
  const dates = state.personalMs?.[dept]?.dates || [];
  const currentTotal = getPersonalMsTargetTotal(dept, member, metric) ?? getLastCumulativeTarget(dates, getPersonalMsTargetMap(dept, member, metric), dept, periodId);
  const input = prompt("\u6700\u7D42\u76EE\u6A19\u5024\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044", currentTotal ? String(currentTotal) : "");
  if (input === null) return;
  const total = Number(input);
  if (!Number.isFinite(total) || total < 0) return;
...
function renderPersonalMsTable(deptKey) {
  ...
  members.forEach((member, index) => {
    ...
    const totalTarget = Number.isFinite(storedTotal) ? num(storedTotal) : getLastCumulativeTarget(dates, targetMap, deptKey, periodId);
    const activeDates = dates.filter((date) => {
      const disabled = isDateBeforeDeptStart(date, deptKey) || isDateAfterDeptEnd(date, deptKey);
      return !disabled;
    });
    ...
    const totalTargetValue = Number.isFinite(storedTotal) ? num(storedTotal) : getLastCumulativeTarget(dates, targetMap, deptKey, periodId) || (totalTarget > 0 ? totalTarget : 0);
    const msCells = dates.map((date, idx) => {
      const isDisabled = isDateBeforeDeptStart(date, deptKey) || isDateAfterDeptEnd(date, deptKey);
      if (isDisabled) return `<td class="ms-cell-disabled"></td>`;
      const value = cumulativeTargets[idx];
...
async function loadAndRenderCompanyMs() {
  ...
  const fallbackIds = allEmployeeIds.length ? allEmployeeIds : getCompanyDailyEmployees().map((emp) => String(emp?.id ?? "")).filter((id) => id);
  const dailyTotals = Object.keys(dailyTotalsFromPayload).length ? dailyTotalsFromPayload : buildCompanyMsDailyTotals(periodId, fallbackIds);
...
function updatePersonalPeriodLabels() {
  ...
  if (dailyLabel) {
    dailyLabel.textContent = dailyPeriod ? `\u8A55\u4FA1\u671F\u9593\uFF1A${formatPeriodMonthLabel(dailyPeriod) || "--"}` : "\u8A55\u4FA1\u671F\u9593\uFF1A-";
  }
}
...
function buildTrendChartConfig(scope) {
  ...
  if (trend && Array.isArray(trend.labels) && Array.isArray(trend.rates) && trend.labels.length) {
    ...
    const fallbackLabels = current.trendMode === "month" ? createTrendDayLabels(current.year, current.month) : DASHBOARD_MONTHS.map((month) => `${month}\u6708`);
