...
async function fetchMasters(client) {
    ...
    return { clients: clientsRes.rows || [], users, csUsers, advisorUsers };
}
        ...
        if (method === "PUT") {
          ...
          `;
             ...
             `, [completeTaskId, candidateId]);
                    ...
                    // (D) 念のため未完了タスクを1件に正規化
                    // 過去データの重複や並行更新で増えた未完了タスクをここで収束させる。
             ...
             `, [candidateId, keepTaskRes.rows[0].id]);
                    ...
                    // (E) candidatesテーブルの同期 (未完了の直近タスクを本体に反映)
                            ...
                            if (s_id) {
                                await client.query(`
	                                    UPDATE candidate_applications SET 
	                                        ...
	                                        -- Note: *_date columns are often DATE while *_at columns are TIMESTAMPTZ.
	                                        -- Do not reuse the same SQL parameter number for both, or Postgres may error:
