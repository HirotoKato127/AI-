...
function renderPersonalSummary(rangeData, monthOverride) {
  const summary = {
    ...
    currentAmount: num(
      monthOverride?.currentAmount ?? monthOverride?.revenue ?? monthOverride?.revenueAmount ?? rangeData?.currentAmount ?? rangeData?.revenue ?? rangeData?.revenueAmount
    ),
    targetAmount: num(
      monthOverride?.targetAmount ?? monthOverride?.revenueTarget ?? rangeData?.targetAmount ?? rangeData?.revenueTarget
    )
  };
...
function renderCompanyMsTable() {
  ...
  MS_DEPARTMENTS.forEach((dept, index) => {
    ...
    const metricCell = isRevenue ? `<th scope="row" class="kpi-v2-sticky-label kpi-v2-ms-metric" rowspan="2">${metricLabel}</th>` : `<th scope="row" class="kpi-v2-sticky-label kpi-v2-ms-metric" rowspan="2">
    ...
    const msAndRateCells = dates.map((date) => {
      ...
      const actual = isRevenue ? state.companyMs.revenue?.actual || 0 : state.companyMs.dailyTotals?.[date]?.[metricOption?.key] || 0;
      let rateDisplay = "-";
      let rateClass = "";
...
function renderPersonalMsTable(deptKey) {
  ...
  members.forEach((member, index) => {
    ...
    const msAndRateCells = dates.map((date) => {
      ...
      const actualValue = savedActual !== void 0 && savedActual !== "" ? Number(savedActual) : autoValue;
      let rateDisplay = "-";
      let rateClass = "";
      if (savedMs && Number(savedMs) > 0) {
    ...
    const actualCells = dates.map((date) => {
      ...
      const autoValue = getAutoCalculatedActual(memberId, date, currentMetricKey);
      const savedActual = state.personalMs[deptKey]?.msActuals?.[memberId]?.[currentMetricKey]?.[date];
      const displayValue = savedActual !== void 0 && savedActual !== "" ? savedActual : autoValue;
      return `
        <td class="ms-actual-cell" colspan="2">
...
async function loadAndRenderCompanyMs() {
  ...
  const dailyTotalsFromPayload = payload?.employees?.length ? buildCompanyMsDailyTotalsFromEmployees(payload.employees, advisorIds) : {};
  const dailyTotals = Object.keys(dailyTotalsFromPayload).length ? dailyTotalsFromPayload : buildCompanyMsDailyTotals(periodId, advisorIds);
