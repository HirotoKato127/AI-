...
function buildTaskIdentityKey(candidateId, date, type) {
  const id = String(candidateId || "");
  const dateKey = toDateKey(date) || "";
  const typeKey = String(type || "").trim();
  return `${id}|${dateKey}|${typeKey}`;
}

function dedupeByTaskIdentity(items, keyBuilder) {
  const seen = new Set();
  const deduped = [];
  (items || []).forEach((item) => {
...
export const handler = async (event) => {
  ...
  try {
    ...
    const tasksAll = dedupeByTaskIdentity(
      tasksAllRaw,
      (task) => buildTaskIdentityKey(task?.candidateId, task?.nextAction?.date, task?.nextAction?.type)
    );
    ...
    if (!isCaller && monthStart && monthEnd) {
      ...
      const dedupeCalendar = (items) => dedupeByTaskIdentity(
        items,
        (item) => buildTaskIdentityKey(item?.candidateId, item?.date, item?.type)
      );
      const dedupedPending = dedupeCalendar(calendarPending);
      const dedupedCompleted = dedupeCalendar(calendarCompleted);
      calendarPending.length = 0;
      calendarPending.push(...dedupedPending);
      calendarCompleted.length = 0;
      calendarCompleted.push(...dedupedCompleted);
    }
