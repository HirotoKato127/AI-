...
const METRIC_SQL = `
  ...
  SELECT c.advisor_user_id,
         COALESCE(ca.proposal_date, ca.recommended_at::date),
         'proposals' AS metric,
         COUNT(*)::int
  FROM candidates c
  JOIN candidate_applications ca ON ca.candidate_id = c.id
  WHERE COALESCE(ca.proposal_date, ca.recommended_at::date) BETWEEN $1 AND $2
  GROUP BY c.advisor_user_id, COALESCE(ca.proposal_date, ca.recommended_at::date)
  UNION ALL
  SELECT c.advisor_user_id,
  ...
  UNION ALL
  SELECT c.advisor_user_id,
         COALESCE(ca.offer_date, ca.offer_at::date),
         'offers' AS metric,
         COUNT(*)::int
  FROM candidates c
  JOIN candidate_applications ca ON ca.candidate_id = c.id
  WHERE COALESCE(ca.offer_date, ca.offer_at::date) BETWEEN $1 AND $2
  GROUP BY c.advisor_user_id, COALESCE(ca.offer_date, ca.offer_at::date)
  UNION ALL
  SELECT c.advisor_user_id,
         COALESCE(ca.offer_accept_date, ca.offer_accepted_at::date),
         'accepts' AS metric,
         COUNT(*)::int
  FROM candidates c
  JOIN candidate_applications ca ON ca.candidate_id = c.id
  WHERE COALESCE(ca.offer_accept_date, ca.offer_accepted_at::date) BETWEEN $1 AND $2
  GROUP BY c.advisor_user_id, COALESCE(ca.offer_accept_date, ca.offer_accepted_at::date)
  UNION ALL
  SELECT c.advisor_user_id,
         p.order_date::date,
         'revenue' AS metric,
  ...
  WHERE p.order_date::date BETWEEN $1 AND $2
  GROUP BY c.advisor_user_id, p.order_date::date
  UNION ALL
  SELECT c.advisor_user_id,
         p.withdraw_date::date,
         'revenue' AS metric,
         (COALESCE(SUM(p.refund_amount), 0) * -1)::int
  FROM placements p
  JOIN candidate_applications ca ON ca.id = p.candidate_application_id
  JOIN candidates c ON c.id = ca.candidate_id
  WHERE p.withdraw_date::date BETWEEN $1 AND $2
  GROUP BY c.advisor_user_id, p.withdraw_date::date
`;
...
export const handler = async (event) => {
  ...
  try {
    ...
    res.rows.forEach(row => {
      ...
      daily[day][row.metric] = Number(daily[day][row.metric] || 0) + Number(row.count || 0);
    });
