...
async function fetchCounts(client, { startDate, endDate, advisorUserId }) {
  const sql = `
    SELECT
      ...
      COUNT(*) FILTER (WHERE COALESCE(ca.proposal_date, ca.recommended_at::date) BETWEEN $1 AND $2) AS proposals,
      ...
      COUNT(*) FILTER (WHERE COALESCE(ca.offer_date, ca.offer_at::date) BETWEEN $1 AND $2) AS offers,
      COUNT(*) FILTER (WHERE COALESCE(ca.offer_accept_date, ca.offer_accepted_at::date) BETWEEN $1 AND $2) AS accepts,
      COUNT(*) FILTER (WHERE COALESCE(ca.join_date, ca.joined_at::date) BETWEEN $1 AND $2) AS hires
    FROM candidates c
