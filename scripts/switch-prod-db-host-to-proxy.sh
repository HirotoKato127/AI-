...
# Update prod Lambda DB_HOST to RDS Proxy endpoint, preserving all other env vars.
#
# Requirements:
...
#
# Usage:
#   scripts/switch-prod-db-host-to-proxy.sh

REGION="ap-northeast-1"
PROXY_ENDPOINT="agentkey-proxy.proxy-cdiqayuosm2o.ap-northeast-1.rds.amazonaws.com"
...
for fn in "${FUNCTIONS[@]}"; do
  ...
  env_payload="$(
    aws lambda get-function-configuration \
      ...
      --output json \
      --no-cli-pager \
    | jq -c --arg host "${PROXY_ENDPOINT}" '{Variables: ((. // {}) + {DB_HOST: $host})}'
  )"
